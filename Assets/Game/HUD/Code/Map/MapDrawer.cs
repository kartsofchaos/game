    ...
    void OnGUI () {
		...
		if (distance > minimapViewRange)
		{
			...
			if (lastSeenInsideViewRange)
				_gameObject.GetComponent<Renderer>().material.SetTexture("_MainTex", navigationTexture);
		...
		{
            if (!lastSeenInsideViewRange)
                _gameObject.GetComponent<Renderer>().material.SetTexture("_MainTex", mainTexture);
